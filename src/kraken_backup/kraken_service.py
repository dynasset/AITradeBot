...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("kraken_service.log"), logging.StreamHandler()],
)
...
class KrakenService:
        ...
        try:
            ...
            if "result" in data:
                ...
                key = (
                    ...
                    else (result_keys[0] if result_keys else None)
                )
    ...
    def place_order(
        ...
        if min_order_size is not None and volume < min_order_size:
            logging.info(
                f"Ordervolume ({volume}) lager dan minimum ({min_order_size}), aanpassen naar minimum."
            )
        ...
        try:
            logging.info(
                f"Order plaatsen: {order_type} {volume} {pair} @ {price} SL: {stoploss} TP: {take_profit}"
            )
                        ...
                        if type_ == "buy":
                            ...
                            for std, aliases in asset_map.items():
                                if quote in aliases and std in balances:
                                    balances[std] = str(
                                        max(0, float(balances[std]) - vol)
                                    )
                                    break
                            else:
                                if quote in balances:
                                    balances[quote] = str(
                                        max(0, float(balances[quote]) - vol)
                                    )
                        elif type_ == "sell":
    ...
    def __init__(self):
        if (
            not KRAKEN_API_KEY
            or not KRAKEN_API_SECRET
            or KRAKEN_API_KEY.strip() == ""
            or KRAKEN_API_SECRET.strip() == ""
        ):
            raise ValueError(
                "Kraken API credentials niet gevonden of leeg in environment."
            )
    ...
    def get_ohlc(self, pair: str, interval: int = 1, limit: int = 100):
        ...
        try:
            ...
            try:
                ...
                logging.warning(
                    f"Kon sommige kolommen niet converteren naar numeriek: {e}"
                )
