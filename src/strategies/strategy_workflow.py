...
# Strategieën registreren (uitbreidbaar)
STRATEGY_MODULES = [
    "src.strategies.scalper",
    # Voeg hier extra strategie-modules toe
]

# Strategie-functies dynamisch laden
def load_strategies():
    strategies = []
...
def fetch_kraken_ohlc_multi(pair, intervals=[1, 3, 5, 15, 60], limit=100):
    """
    Haal OHLC data op voor meerdere timeframes per pair via KrakenService.
    Returns: dict {interval: df}
    """
    ks = KrakenService()
    dfs = {}
...
def main():
    # Credentials/config should be loaded from a config file or environment variables in production
    ...
    TELEGRAM_CHAT_ID = 1577940521  # Gebruiker chat_id
    ...
    for pair in list(asset_pairs.keys())[:10]:
        ...
        for strategy in strategies:
            ...
            if res and res.get('kans'):
                ...
                proposal_id = f"{pair}_{strategy.__name__}_{int(pd.Timestamp.now().timestamp())}"
                msg = (
                ...
                if approved:
                    ...
                    telegram.send_message(f"Order NIET geplaatst voor {pair} ({strategy.__name__})")
                ...
                print(f"Voorspelling op {pair} ({strategy.__name__}): kans mogelijk binnen {res['voorspelling']['minuten']} minuten.")
            else:
